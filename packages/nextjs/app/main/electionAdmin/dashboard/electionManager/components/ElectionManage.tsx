...
const ElectionManage = () => {
    ...
    if (orgID) {
      ...
      const fetchData = () => {
        try {
          ...
          console.log("Wave: " + waveNumber?.toString())
          const electionMode = electionDetail?.[8]
  ...
  }, [ElectionList, orgID, adminAddress]);
  ...
  const handleButtonClick = async (
    ...
    electionID: string,
    status: string,
    isHaveCandidateMode: boolean
  ) => {
    e.stopPropagation();
    try {
      console.log('Candidate Mode? : ' + isHaveCandidateMode)
      if (status === "Dalam Persiapan") {
        ...
        const isSingleorFinalWaveElection = candidates.length === 2;
        console.log("iS Final or Single Wave? : " + isSingleorFinalWaveElection)
        console.log("passed candidates: " + passingCandidates.length)
        ...
        try {
          if (isSingleorFinalWaveElection) {
            // Jika hanya 2 kandidat tersisa, langsung simpan ke ElectionResult
            await VOXCommand(
              {
              ...
              },
              {
                onBlockConfirmation: txnReceipt => {
                  toast.success(`Pemilihan Berhasil Diselesaikan: ${electionID} - ${txnReceipt.cumulativeGasUsed}`, {
                    autoClose: 3000,
                    onClose: () => {
                      // Reset state kandidat di frontend
                      setSelectedElection(prev => ({
                        ...prev!,
                        candidateNames: isSingleorFinalWaveElection
                          ? [] // Kosongkan kandidat jika pemilu selesai
                          : passingCandidates.map(candidate => candidate.candidateName),
                        voteCounts: isSingleorFinalWaveElection
                          ? [] // Kosongkan suara kandidat jika selesai
                          : passingCandidates.map(() => 0),
                        candidateIDs: isSingleorFinalWaveElection
                          ? [] // Kosongkan ID kandidat jika selesai
                          : passingCandidates.map(candidate => BigInt(candidate.candidateID)),
                      }));
                    },
                  });
                },
          ...
          } else if (!isSingleorFinalWaveElection) {
            // Jika lebih dari 2 kandidat tersisa, reset state dan eliminasi kandidat
  ...
  const handleVoteClick = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`Error saat voting: ${error}`);
    }
  };
                  ...
                  <td className="py-2 px-4 border-b text-center text-sm md:text-md">
                    {election.electionStatus !== "Pemilihan Selesai" && (
                      <button
                        ...
                        onClick={(e) =>
                          handleButtonClick(e, election.electionID, election.electionStatus, electionDetail?.[9] as boolean)
                        }
                      >
                  ...
                  selectedElection.electionMode ? (
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="input input-bordered mx-auto w-1/2"
                      />
                    </div>
                  )
                ) : (
